...
while gui.running:
    ...
    ells = [Ellipse(xy=pos[i,:],
                    width=r[i]*0.003, height=r[i]*0.002*(1-0.6*sin(np.random.rand())),
                    angle=np.random.rand() * 360)
            for i in range(NUM)]
    t=str(step)
    fig, ax = plt.subplots(subplot_kw={'aspect': 'equal'}, dpi=400)
    for e in ells:
        ax.add_artist(e)
